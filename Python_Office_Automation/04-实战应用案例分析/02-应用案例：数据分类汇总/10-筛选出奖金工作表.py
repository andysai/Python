...
def shaixuan(workbook, get_value, str_name):
    ...
    if str_name == "薪资":
        for list_value_inner in list_value_outer:
            if str(list_value_inner[3]) == get_value + ".0":
                # 创建薪资表格对象，并写入值
                xinzi_sht = workbook.sheets["薪资"]

                # 先写第一行数据
                xinzi_sht.range("A1:E1").value = merge_sht.range("A1:E1").value

                # 拼接字符串
                str_name_value = str(int(xinzi_sht.used_range.address.split("$")[-1]) + 1)
                str_address_value = "A" + str_name_value + ":" + "E" + str_name_value
                xinzi_sht.range(str_address_value).value = list_value_inner

    # 按照奖金筛选
    if str_name == "奖金":
        for list_value_inner in list_value_outer:
            if str(list_value_inner[4]) == get_value + ".0":
                # 创建奖金表格对象，并写入值
                jiangjin_sht = workbook.sheets["奖金"]

                # 先写第一行数据
                jiangjin_sht.range("A1:E1").value = merge_sht.range("A1:E1").value

                # 拼接字符串
                str_name_value = str(int(jiangjin_sht.used_range.address.split("$")[-1]) + 1)
                str_address_value = "A" + str_name_value + ":" + "E" + str_name_value
                jiangjin_sht.range(str_address_value).value = list_value_inner
...
if __name__ == "__main__":
    ...
    # 保存退出
    workbook.save("../source_material/02/数据筛选(1).xlsx")
    workbook.close()
    app.quit()
